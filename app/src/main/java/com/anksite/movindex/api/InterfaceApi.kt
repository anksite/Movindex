...
interface InterfaceApi {
    ...
    @GET("/3/discover/movie/{id}?api_key=${BuildConfig.KEY}")
    suspend fun movie(
        @Path("id") id: String
    ): Result<ResponseMovie>

    @GET("/3/discover/movie/{id}/reviews?api_key=${BuildConfig.KEY}&page=1")
    suspend fun reviews(
        @Path("id") id: String
    ): Result<ResponseReview>

    @GET("/3/discover/movie/{id}/videos?api_key=${BuildConfig.KEY}")
    suspend fun video(
        @Path("id") id: String
